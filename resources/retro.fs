...
vec4 sample(vec2 co) {
    vec4 sum = vec4(0.0);
    vec2 hstep = vec2(0.0015, 0.0);
    vec2 vstep = vec2(0.0, 0.0015);
...
void main(void){
    ...
    outcolor.r = mix(0.5 * red, red, wavePos);
    outcolor.g = mix(0.5 * green, green, wavePos);
    outcolor.b = mix(0.5 * blue, blue, wavePos);
}
