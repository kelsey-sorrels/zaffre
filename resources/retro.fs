...
uniform float time, noise, colorShift, scanlineDepth, brightness, contrast, gamma;
...
void main(void){
    ...
    float red   = sample(vec2(colorShift, 0.0)).x;
    float green = sample(vec2( 0.0000, 0.0)).y;
    float blue  = sample(vec2(-colorShift, 0.0)).z;

    vec3 rawcolor;
    
    rawcolor.r = mix(scanlineDepth * red, red, wavePos);
    ...
    //outcolor = Gamma(brightnessContrast(noise * noise1(time + ) + rawcolor, brightness, contrast), gamma);
    outcolor = brightnessContrast(rawcolor + n);
}
